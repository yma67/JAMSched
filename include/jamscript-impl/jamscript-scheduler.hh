...
struct RealTimeTaskScheduleEntry {
    ...
    bool inside(uint64_t timePoint) const {
        return ((startTime * 1000) <= timePoint) && 
               (timePoint <= (endTime * 1000));
    }
};
 ...
 *         and vice verca, otherwise, we DispatchTask according to virtual clock value
 ...
 * @remark when we DecideNextScheduleToRun to schedule interactive task, if there is any expired task from priority queue, we put them into stack, 
...
#define this_scheduler() (static_cast<JAMScript::Scheduler*>(          \
...
public:
    ...
    std::shared_ptr<CFuture> 
    CreateInteractiveTask(uint64_t, uint64_t, void *, void(*f)(CTask*, void*));
    Scheduler(std::vector<RealTimeTaskScheduleEntry> normalSchedule,
                     std::vector<RealTimeTaskScheduleEntry> greedySchedule,
                     uint32_t deviceId, uint32_t stackSize, 
                     void* localAppArgs,
...
private:
    ...
    std::mutex futureMutex, namedExecMutex;
    std::unique_ptr<SporadicTaskManager> sporadicManagers[2];
    std::vector<RealTimeTaskScheduleEntry>* currentSchedule;
    decltype(std::chrono::high_resolution_clock::now()) 
    taskStartTime, cycleStartTime, schedulerStartTime;
...
public:
    ...
    static void 
    LocalNamedTaskFunctionBatchRealtime(CTask* self, void* args) {
        {
            auto* exec_p = 
    ...
    template <typename Tr, typename Tf>
    static void
    LocalNamedTaskFunctionInteractive(CTask* self, void* args) {
        {
    ...
    std::shared_ptr<CFuture> 
    CreateLocalNamedTaskAsync(uint64_t deadline, 
        ...
        return CreateInteractiveTask(deadline, duration, 
                                    ...
                                    LocalNamedTaskFunctionInteractive
                                    <Tr, decltype(exec_fp)>);
    }
    template <typename Tr, typename ...Args> 
    std::shared_ptr<Future<Tr>> 
    CreateLocalNamedTaskAsync(uint32_t taskId, std::string exec_name, 
                               Args&& ...args) {
        if (local_function_map.find(exec_name) == local_function_map.end()) {
            return nullptr;
        }
        ...
        CreateRealTimeTask(taskId, new std::pair<std::shared_ptr<Future<Tr>>, 
    ...
    template <typename Tr, typename ...Args> 
    std::shared_ptr<Future<Tr>> 
    CreateLocalNamedTaskAsync(uint64_t burst, std::string exec_name, 
                               Args&& ...args) {
        if (local_function_map.find(exec_name) == local_function_map.end()) {
            return nullptr;
        }
        ...
        auto exec_fp = std::bind(named_exec_fp, std::forward<Args>(args)...);
        auto task_handle = std::make_shared<Future<Tr>>();
        CreateBatchTask(burst, new std::pair<std::shared_ptr<Future<Tr>>, 
                       decltype(exec_fp)>(task_handle, std::move(exec_fp)),
                       LocalNamedTaskFunctionBatchRealtime<Tr, decltype(exec_fp)>);
        return task_handle;
    }
