...
namespace JAMScript {
    namespace ThisTask {
        ...
        void SleepUntil(TimePoint tp) {
            dynamic_cast<RIBScheduler*>(thisTask->baseScheduler)
                ->timer.SetTimeoutUntil(thisTask, tp);
        }
        void SleepFor(Duration dt, std::unique_lock<SpinLock>& lk, Notifier* f) {
            dynamic_cast<RIBScheduler*>(thisTask->baseScheduler)
                ->timer.SetTimeoutFor(thisTask, dt, lk, f);
        }
        void SleepUntil(TimePoint tp, std::unique_lock<SpinLock>& lk, Notifier* f) {
            dynamic_cast<RIBScheduler*>(thisTask->baseScheduler)
                ->timer.SetTimeoutUntil(thisTask, tp, lk, f);
        }
        void Yield() {
    ...
    void TaskInterface::ExecuteC(uint32_t tsLower, uint32_t tsHigher) {
        TaskInterface* task = reinterpret_cast<TaskInterface*>(
            tsLower | ((static_cast<uint64_t>(tsHigher) << 16) << 16));
        task->Execute();
