...
void playback(task_t* self, void* args) {
    ...
    make_task(capt, &scheduler, capture, memset, &capsleep, NULL, 256 * 1024);
    fifo.push(capt);
    finish_task(self, 0);
}
...
void process(task_t* self, void* args) {
    ...
    make_task(play, &scheduler, playback, memset, &playbacksleep, NULL, 256 * 1024);
    fifo.push(play);
    finish_task(self, 0);
}
...
void capture(task_t* self, void* args) {
    ...
    make_task(processing, &scheduler, process, memset, &procsleep, NULL, 256 * 1024);
...
int main() {
    ...
    make_task(flame, &scheduler, capture, memset, &capsleep, NULL, 256 * 1024);
    fifo.push(flame);
