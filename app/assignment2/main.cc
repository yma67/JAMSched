...
using ReadWriteLock = ReaderWriterWritePriortized;
...
int main() {
    ...
    ribScheduler.CreateBatchTask({false, 1024 * 256}, std::chrono::high_resolution_clock::duration::max(), [&]() {
        ...
        for (int i = 0; i < 5000; i++) {
            rpool.push_back(ribScheduler.CreateBatchTask({true, 0, true}, std::chrono::high_resolution_clock::duration::max(), [&](int ntry) {
        ...
        for (int i = 0; i < 100; i++) {
            wpool.push_back(ribScheduler.CreateBatchTask({true, 0, true}, std::chrono::high_resolution_clock::duration::max(), [&](int ntry) {
                for (int i = 0; i < ntry; i++) {
                    rw.WriteLock();
                    syncVar++;
                    assert(syncVar <= 1);
